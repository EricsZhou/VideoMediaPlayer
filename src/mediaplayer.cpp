...
float MediaPlayer::ffp_get_property_float(int id, float default_value)
{
    return 0;
}

void MediaPlayer::ffp_set_property_float(int id, float value)
{
    switch (id) {
...
/* seek in the stream */
void MediaPlayer::stream_seek(VideoState *is, int64_t pos, int64_t rel)
{
    if (!is->seek_req) {
        is->seek_pos = pos;
        is->seek_rel = rel;
...
void MediaPlayer::ffp_set_playback_rate(float rate)
{
    pf_playback_rate = rate;
    pf_playback_rate_changed = 1;
}

float MediaPlayer::ffp_get_playback_rate()
{
    return pf_playback_rate;
}

int MediaPlayer::ffp_get_playback_rate_change()
{
    return pf_playback_rate_changed;
}

void MediaPlayer::ffp_set_playback_rate_change(int change)
{
    pf_playback_rate_changed = change;
}

int64_t MediaPlayer::get_target_frequency()
{
    if(m_CurStream)
    {
...
int MediaPlayer::get_target_channels()
{
    if(m_CurStream)
    {
...
int MediaPlayer::is_normal_playback_rate()
{
...
void MediaPlayer::video_refresh(void *opaque, double *remaining_time)
{
...
int MediaPlayer::get_video_frame(VideoState *is, AVFrame *frame)
{
    int got_picture;
...
void MediaPlayer::update_sample_display(VideoState *is, short *samples, int samples_size)
{
    int size, len;
...
void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    MediaPlayer *pMediaPlayer = MediaPlayer::GetInstance();
    ...
    while (len > 0) {
        ...
        if (is->audio_buf_index >= is->audio_buf_size) {        // 数据已经处理完毕了，需要读取新的
            audio_size = pMediaPlayer->audio_decode_frame(is);
//            qDebug() << "1 audio_buf_size: " << audio_size;
            ...
            // 2 是否需要做变速
            if(pMediaPlayer->ffp_get_playback_rate_change())
            {
                pMediaPlayer->ffp_set_playback_rate_change(0);
                // 初始化
                if(pMediaPlayer->audio_speed_convert)
                {
                    // 先释放
                    sonicDestroyStream(pMediaPlayer->audio_speed_convert);

                }
                // 再创建
                pMediaPlayer->audio_speed_convert = sonicCreateStream(pMediaPlayer->get_target_frequency(),
                                                                   pMediaPlayer->get_target_channels());

                // 设置变速系数
                sonicSetSpeed(pMediaPlayer->audio_speed_convert, pMediaPlayer->ffp_get_playback_rate());
                sonicSetPitch(pMediaPlayer->audio_speed_convert, 1.0);
                sonicSetRate(pMediaPlayer->audio_speed_convert, 1.0);
            }
            if(!pMediaPlayer->is_normal_playback_rate() && is->audio_buf)
            {
                // 不是正常播放则需要修改
                ...
                {
                    out_ret = sonicWriteFloatToStream(pMediaPlayer->audio_speed_convert,
                                                      (float *)is->audio_buf,
                                                      actual_out_samples);
                }
                else  if(is->audio_tgt.fmt == AV_SAMPLE_FMT_S16)
                {
                    out_ret = sonicWriteShortToStream(pMediaPlayer->audio_speed_convert,
                                                      (short *)is->audio_buf,
                                                      actual_out_samples);
                }
                else
                ...
                num_samples =  sonicSamplesAvailable(pMediaPlayer->audio_speed_convert);
                // 2通道  目前只支持2通道的
                ...
                if(out_ret)
                {
                    // 从流中读取处理好的数据
                    if(is->audio_tgt.fmt == AV_SAMPLE_FMT_FLT) {
                        sonic_samples = sonicReadFloatFromStream(pMediaPlayer->audio_speed_convert,
                                                                 (float *)is->audio_buf1,
                                                                 num_samples);
                    }
                    else  if(is->audio_tgt.fmt == AV_SAMPLE_FMT_S16)
                    {
                        sonic_samples = sonicReadShortFromStream(pMediaPlayer->audio_speed_convert,
                                                                 (short *)is->audio_buf1,
                                                                 num_samples);
                    }
                    else
...
//                    qDebug() << "mdy audio_buf_size: " << is->audio_buf_size;
    ...
    if (!std::isnan(is->audio_clock)) {
        double audio_clock = is->audio_clock / pMediaPlayer->ffp_get_playback_rate();
        pMediaPlayer->set_clock_at(&is->audclk, audio_clock  - (double)(2 * is->audio_hw_buf_size + is->audio_write_buf_size) / is->audio_tgt.bytes_per_sec, is->audio_clock_serial, audio_callback_time / 1000000.0);
        pMediaPlayer->sync_clock_to_slave(&is->extclk, &is->audclk);
    }
}

int MediaPlayer::audio_open(void *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate,
...
//打开流
int MediaPlayer::stream_component_open(VideoState *is, int stream_index)
{
    AVFormatContext *ic = is->ic;
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        is->auddec.decode_thread = std::thread(&MediaPlayer::audio_thread, this, is);
        SDL_PauseAudio(0);
        break;
    case AVMEDIA_TYPE_VIDEO:
        ...
        is->viddec.decode_thread = std::thread(&MediaPlayer::video_thread, this, is);
    ...
    case AVMEDIA_TYPE_SUBTITLE:
        ...
        is->subdec.decode_thread = std::thread(&MediaPlayer::subtitle_thread, this, is);
        break;
    default:
        break;
    }
...
//读取线程
void MediaPlayer::ReadThread(VideoState *is)
{
    //VideoState *is = (VideoState *)arg;
...
VideoState* MediaPlayer::stream_open(const char *filename)
{
    VideoState *is;
    //构造视频状态类
    ...
    //构建读取线程
    is->read_tid = std::thread(&MediaPlayer::ReadThread, this, is);
...
void MediaPlayer::stream_cycle_channel(VideoState *is, int codec_type)
{
    AVFormatContext *ic = is->ic;
...
void MediaPlayer::refresh_loop_wait_event(VideoState *is, SDL_Event *event) {
...
//播放控制循环
void MediaPlayer::LoopThread(VideoState *cur_stream)
{
    SDL_Event event;
...
void MediaPlayer::OnPlaySeek(double dPercent)
{
...
void MediaPlayer::OnPlayVolume(double dPercent)
{
...
void MediaPlayer::OnSeekForward()
{
    if (m_CurStream == nullptr)
    {
        return;
...
void MediaPlayer::UpdateVolume(int sign, double step)
{
    if (m_CurStream == nullptr)
    {
        return;
...
void MediaPlayer::do_exit(VideoState* &is)
{
    if (is)
    {
        stream_close(is);
...
bool MediaPlayer::ConnectSignalSlots()
{
    connect(this, &MediaPlayer::SigStop, &MediaPlayer::OnStop);

    return true;
}


MediaPlayer *MediaPlayer::m_pInstance = new MediaPlayer();
...
bool MediaPlayer::StartPlay(QString strFileName, WId widPlayWid)
{
    m_bPlayLoop = false;
    ...
    //事件循环
    m_tPlayLoopThread = std::thread(&MediaPlayer::LoopThread, this, is);
